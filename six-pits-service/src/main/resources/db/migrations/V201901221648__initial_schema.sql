...
create table player (
  ...
  player_creation timestamp default CURRENT_TIMESTAMP(),
  player_name varchar(255) unique not null,
  player_hash varchar(255) not null,
  player_status_id integer default 1,
...
create table game (
  ...
  foreign key (player_id_1) references player(player_id) on delete cascade,
  foreign key (player_id_2) references player(player_id) on delete cascade
);

create table score (
  score_id integer not null primary key auto_increment,
  score_creation timestamp default CURRENT_TIMESTAMP(),
  score_points integer default 1,
